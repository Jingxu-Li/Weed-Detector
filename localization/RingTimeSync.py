...
data_unsync = np.array([[3, 0, 0], [3, 0, 0], [1, 0, 0], [1, 0, 0], [27, 0, 0],
                        [26, 0, 0], [50, 0, 0], [2, 0, 0], [1, 0, 0],
                        [5, 0, 0], [1, 0, 0], [22, 0, 0], [20, 0, 0]])
        ...
        while True:
            ...
            if counter_j < len(t_unsync) - 1:
                ...
                if pos_j_front == 0 and pos_j_back == 0:  # Append extra t_unsync for comparison
                    ...
                    unsync_buffer.append(
                        t_unsync[counter_j])  # Append t_unsyc[0]
                ...
                if unsync_buffer.data[pos_j_front] > std_buffer.data[
                        pos_i] and unsync_buffer.data[
                            pos_j_back] > std_buffer.data[pos_i]:
                ...
                if unsync_buffer.data[pos_j_front] < std_buffer.data[
                    ...
                    front_diff = std_buffer.data[pos_i] - unsync_buffer.data[
                        pos_j_front]
                    back_diff = unsync_buffer.data[
                        pos_j_back] - std_buffer.data[pos_i]
                    diff = unsync_buffer.data[pos_j_back] - unsync_buffer.data[
                        pos_j_front]
                    ...
                    if front_diff < threshold and back_diff < threshold:
                        ...
                        sync_x = front_scale * data_unsync[counter_data][
                            0] + back_scale * data_unsync[counter_data + 1][0]
                        sync_y = front_scale * data_unsync[counter_data][
                            1] + back_scale * data_unsync[counter_data + 1][1]
                        sync_z = front_scale * data_unsync[counter_data][
                            2] + back_scale * data_unsync[counter_data + 1][2]
                        ...
                        counter_data += 1
                        unsync_buffer.append(
                            t_unsync[counter_j])  # Append new t_unsync
                ...
                if unsync_buffer.data[pos_j_front] < std_buffer.data[
                    ...
                    unsync_buffer.append(
                        t_unsync[counter_j]
                    )  # Append a new t_unsync to the buffer
                ...
                if unsync_front > std_buffer.data[
                ...
                if unsync_front < std_buffer.data[
                    ...
                    if front_diff < threshold and back_diff < threshold:
                        ...
                        #unsync_buffer.data[pos_j_back], std_buffer.data[pos_i])
                        sync_x = front_scale * data_unsync[counter_data - 1][
                            0] + back_scale * data_unsync[counter_data][0]
                        sync_y = front_scale * data_unsync[counter_data - 1][
                            1] + back_scale * data_unsync[counter_data][1]
                        sync_z = front_scale * data_unsync[counter_data - 1][
                            2] + back_scale * data_unsync[counter_data][2]
                        sync.append([sync_x, sync_y, sync_z])
                        # Move position pointer
                        pos_i += 1
                        if pos_i > std_size - 1:
        ...
        while True:
            ...
            if unsync_buffer.data[pos_j_front] < std and unsync_buffer.data[
                ...
                if counter_j < len(t_unsync) - 1:
                    ...
                    unsync_buffer.append(
                        t_unsync[counter_j]
                    )  # Append the last t_unsync to the buffer
                    ...
                    if front < std and back > std:
                        ...
                        if front_diff < threshold and back_diff < threshold:
                            #print("checkpoint 20", unsync_buffer.data[pos_j_front],
                            #unsync_buffer.data[pos_j_back], std_buffer.data[pos_i])
                            sync_x = front_scale * data_unsync[
                                counter_data -
                                1][0] + back_scale * data_unsync[counter_data][
                                    0]
                            sync_y = front_scale * data_unsync[
                                counter_data -
                                1][1] + back_scale * data_unsync[counter_data][
                                    1]
                            sync_z = front_scale * data_unsync[
                                counter_data -
                                1][2] + back_scale * data_unsync[counter_data][
                                    2]
                            sync.append([sync_x, sync_y, sync_z])
